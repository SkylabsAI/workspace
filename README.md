...
make clone-only-public -j # Cloning only the publicly-accessible repos.
...
directories listed above). Here is a list of commonly-used such targets:
...
- `make psi-peek` runs `git status` in sub-repos of the `psi/` directory.
